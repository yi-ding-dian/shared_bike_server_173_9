...
int NetworkInterface::m_timeoutWrite = 0;
int NetworkInterface::m_timeoutRead = 0;
...
void NetworkInterface::listener_cb(evconnlistener * listener, evutil_socket_t fd, sockaddr * sock, int socklen, void * arg)
{
	...
	bufferevent_settimeout(bev, m_timeoutRead, m_timeoutWrite);//超时值应设置在配置文件中
}
...
void NetworkInterface::setTimeout(int timeout_read, int timeout_write)
{
	m_timeoutRead = timeout_read;
	m_timeoutWrite = timeout_write;
}
